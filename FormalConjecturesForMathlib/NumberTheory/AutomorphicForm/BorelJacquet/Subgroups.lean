...
* `integralAdeles` and `integralAdelicSubgroup`: the integral adeles `Ẑ` and the compact open
  subgroup `GL n Ẑ` of `G(𝔸_f)` (`isOpen_integralAdelicSubgroup`, `isCompact_integralAdelicSubgroup`),
...
(`isOpen_integralAdelicSubgroup`, `isCompact_integralAdelicSubgroup`), as condition (b1) requires. -/
...
lemma isOpen_integralAdelicSubgroup : IsOpen ((integralAdelicSubgroup n) : Set (GL n 𝔸ᶠ[ℤ, ℚ])) := by
...
lemma isCompact_integralAdelicSubgroup : IsCompact ((integralAdelicSubgroup n) : Set (GL n 𝔸ᶠ[ℤ, ℚ])) := by
