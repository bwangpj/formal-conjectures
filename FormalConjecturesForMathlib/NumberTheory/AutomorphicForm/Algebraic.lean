...
-- `Matrix (Fin m) (Fin m) ℂ` is a Lie ring under the commutator; Mathlib keeps this instance
-- local, as in `BorelJacquet.CenterAction`, so each file that needs it enables it itself.
