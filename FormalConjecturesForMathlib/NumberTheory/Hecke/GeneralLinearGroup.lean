...
the Satake parameters. The two examples in mind are `GL n ℤ_[p] ≤ GL n ℚ_[p]`, giving the
spherical Hecke algebra at `p`, and `GL n ℤ ≤ GL n ℚ`, the classical Hecke pair of modular
forms.
...
for a ring like `k[[t]]` with `k` infinite.
